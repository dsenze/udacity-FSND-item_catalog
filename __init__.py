...
# If project gets deployed in Apache, fullname url to file is needed.
...
# add itemid in url.
app.config['UPLOAD_FOLDER'] = '/var/www/itemcatalog/itemcatalog/uploads/'


def moveFile(filename, directory, itemid):
    path = os.path.join("/var/www/itemcatalog/itemcatalog/uploads/", filename)
    target = os.path.join('/var/www/itemcatalog/itemcatalog/static/images/' +
                          directory + '/' + str(itemid) + '/', filename)
    targetDir = '/var/www/itemcatalog/itemcatalog/static/images/' + \
